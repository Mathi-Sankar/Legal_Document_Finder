...
def query(payload):
    ...
    headers = {
        "Authorization": f"Bearer {os.getenv('HUGGINGFACE_KEY')}",
    ...
    try:
        ...
        return {"answer": "Connection failed"}, 0
    ...
    try:
        data = response.json()
        print("HF Response:", data)   # 👈 VERY IMPORTANT DEBUG
    except Exception as e:
        print("JSON Error:", e)
        return {"answer": "Invalid response from model"}, responseTime
